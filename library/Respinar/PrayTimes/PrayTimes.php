...
class PrayTimes
{
    ...
    */
    ...
    function getPrayerTimes($timestamp, $latitude, $longitude, $timeZone,$resulttimes)
    {
        $date = @getdate($timestamp);
        $times = $this->getDatePrayerTimes($date['year'], $date['mon'], $date['mday'],
                    $latitude, $longitude, $timeZone);

        $abc = array_combine($this->timeNames,$times);
		$result = array();
        foreach($resulttimes as $key) {
			$result[$key] = $abc[$key];
		}
